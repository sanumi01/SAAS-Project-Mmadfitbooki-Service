...
function findInteractiveMatches(content) {
  const matches = [];
  // match tags like <button ...> or <input ... /> across lines
  const tagRegex = /<\s*(button|input|textarea|select|a)\b[\s\S]*?>/gi;
  let m;
  while ((m = tagRegex.exec(content)) !== null) {
    const tag = m[0];
    const index = m.index;
    // ignore anchors that are clearly navigation-only with href and no role/onClick (we'll still report)
    if (!/focus-ring/.test(tag)) {
      matches.push({ index, tag });
    }
  }

  // also find non-semantic clickable containers with onClick (div, span, li, etc.)
  const onclickRegex = /<\s*([a-zA-Z0-9_-]+)\b[\s\S]*?onClick=|role=\"button\"/gi;
  while ((m = onclickRegex.exec(content)) !== null) {
    const index = m.index;
    // capture surrounding tag
